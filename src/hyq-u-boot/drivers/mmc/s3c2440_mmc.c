...
#include "s3c2440_mmc.h"

#if defined(CONFIG_MMC) && defined(CONFIG_S3C2440_MMC)

static struct s3c2440_sdi *sdi;
static block_dev_desc_t mmc_dev;

/*FIXME needs to read cid and csd info to determine block size and other parameters*/
static uchar mmc_buf[MMC_BLOCK_SIZE];
static sd_csd_t sd_csd;
...
static int mmc_block_read(uchar *dst, ulong src, ulong len)
{
	...
	/* send read command */
	resp = mmc_cmd(MMC_CMD_READ_SINGLE_BLOCK, (mmc_dev.if_type == IF_TYPE_SDHC) ? (src >> 9) : src, CMD_F_RESP);
...
备    注：无。
********************************************************************/
static u_int32_t mmc_size(sd_csd_t *csd)
{
...
int mmc_legacy_init(int verbose)
{
	...
	sdi = s3c2440_get_base_sdi();

	debug("mmc_init(PCLK=%u)\n", get_PCLK());

/***************设置时钟、分频、块大小、超时、中断、控制*****************/
...
/***************** CMD8--检查接口条件，判断是SD还是SDHC *******************/
	/* Send supported voltage range */
	/* SD cards 1.x do not answer to CMD8 */
    resp = mmc_cmd(MMC_CMD_SEND_EXT_CSD, ((1 << 8) | 0xAA), CMD_F_RESP_R7);
    if (!resp[0]) 
    {
...
/**************************** CMD3--设置地址 ****************************/
		...
		resp = mmc_cmd(MMC_CMD_SET_RELATIVE_ADDR, MMC_DEFAULT_RCA, CMD_F_RESP);
		if (is_sd)
			rca = resp[0] >> 16;
...
/**************************** CMD9--获取CSD ****************************/
		...
		if (resp) 
        {
			sd_csd_t *csd = (sd_csd_t *)resp;
			memcpy(&sd_csd, csd, sizeof(csd));
			rc = 0;
			mmc_ready = 1;
