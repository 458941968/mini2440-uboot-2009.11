...
#ifdef CONFIG_SYS_DIRECT_NAND_TFTP         //==add by hyq start
    char cmd_buf[200];

    if(strcmp(tftp_filename,"rootfs.yaffs2")==0)
    {
        printf("download rootfs");
        sprintf(cmd_buf, "nand write.yaffs2 %x %x %x", (char *)src, (ulong)(load_addr+offset), len);
        run_command(cmd_buf, 0);
    }
    else
#endif /* CONFIG_SYS_DIRECT_NAND_TFTP */    //==add by hyq end
